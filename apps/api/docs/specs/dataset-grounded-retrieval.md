...
- Query normalization lives in `src/core/service/text_normalizer_service.py`; product-catalog row mapping and searchable-field knowledge live under `src/features/product/infrastructure/output/persistence/product_catalog_repository.py`, while catalog SQL/readiness access lives under `src/features/product/infrastructure/output/persistence/product_catalog_reader.py`.
